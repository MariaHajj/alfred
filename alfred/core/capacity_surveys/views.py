...
from alfred.core.capacity_surveys.forms import SurveyForm
...
def survey_register(id):
    ...
    if StudentsRegisteredInSurveys.query.filter_by(survey_id=survey.id,
                                                   student_id=current_user.id).first():  # noqa
        student_submitted = True

    if form.validate_on_submit():
        survey.number_of_requests += 1
        register_student_in_survey = StudentsRegisteredInSurveys(survey_id=survey.id,         # noqa
                                                                 student_id=current_user.id)  # noqa
    ...
    return render_template('surveys.html', survey=survey, form=form,
                           student_submitted=student_submitted, title='survey')
